...
export const SeveritySchema = z.enum(['critical', 'serious', 'moderate', 'minor'])
  .describe('Impact level: critical (blocks users), serious (significant barrier), moderate (some difficulty), minor (annoyance)');
...
export const WCAGPrincipleSchema = z.enum([
  ...
  'understandable',
  'robust'
]).describe('WCAG principle: perceivable (can sense), operable (can use), understandable (can comprehend), robust (works with assistive tech)');
...
export const WCAGCriterionSchema = z.string()
...
export const CombinedAnalysisResultSchema = z.object({
  ...
  individualResults: z.array(AnalysisResultSchema).optional()
