...
const BrowserOptionsSchema = z.object({
  ...
  waitForTimeout: z.number().int().positive().max(60000).optional().describe('Time to wait in ms before analysis (max 60s)'),
...
export const Pa11yToolInputSchema = z.object({
  url: UrlSchema.optional(),
  html: HtmlSchema.optional(),
  options: z.object({
    standard: z.enum(['WCAG2A', 'WCAG2AA', 'WCAG2AAA', 'WCAG21A', 'WCAG21AA', 'WCAG21AAA'])
      .default('WCAG21AA')
...
export const ESLintA11yToolInputSchema = z.object({
...
export const CombinedAnalysisInputSchema = z.object({
  ...
  tools: z.array(z.enum(['axe-core', 'pa11y', 'eslint-vuejs-a11y']))
    .min(1)
  ...
  (data) => data.url !== undefined || data.html !== undefined || (data.files !== undefined && data.files.length > 0),
