...
import {
  ...
  getRequiredRatio,
  suggestFixedColor,
} from '@/utils/color-analysis/index.js';
import { rgbToHex } from '@/utils/color-analysis/converters.js';
import type { RGB } from '@/types/color-analysis.js';

export interface ContrastAdapterConfig extends AdapterConfig {
  headless?: boolean;
  browserArgs?: string[];
  ignoreHTTPSErrors?: boolean;
}

export interface ContrastAdapterOptions extends AnalysisOptions {
...
export class ContrastAdapter extends BaseAdapter {
  readonly name = 'contrast-analyzer';
  readonly version = '1.0.0';

  private browser: Browser | null = null;
  private contrastConfig: ContrastAdapterConfig;
  ...
  private async ensureBrowser(): Promise<void> {
    if (!this.browser || !this.browser.connected) {
      this.logger.debug('Launching browser', {
        ignoreHTTPSErrors: this.contrastConfig.ignoreHTTPSErrors,
      });

      const baseArgs = this.contrastConfig.browserArgs ?? ['--no-sandbox', '--disable-setuid-sandbox'];
      const sslArgs = this.contrastConfig.ignoreHTTPSErrors
        ? ['--ignore-certificate-errors', '--ignore-ssl-errors', '--allow-running-insecure-content']
        : [];

      this.browser = await puppeteer.launch({
        headless: this.contrastConfig.headless ?? true,
        args: [...baseArgs, ...sslArgs],
        acceptInsecureCerts: this.contrastConfig.ignoreHTTPSErrors ?? false,
      });
    }
  }
